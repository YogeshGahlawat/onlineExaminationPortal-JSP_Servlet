...
public class userMail extends HttpServlet{
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
        ...
        try{
            ...
            PreparedStatement psmt = conn.prepareStatement("select * from user where userMail = ?");
            psmt.setString(1, email);
            ResultSet rs = psmt.executeQuery();
            while(rs.next()){
                if(email.equals(rs.getString("userMail"))){
                    ...
                    try{
                        ...
                        msg.setSubject("RESET USER PASSWORD");
                        msg.setContent(
                            "THIS EMAIL IS REGARDING TO RESET USER PASSWORD, AND THIS EMAIL BELONGS TO " + email +
                            ", CLICK HERE TO <a href='http://localhost:8080/onlineExaminationPortal/resetUserPassword.html'>RESET USER PASSWORD</a>", "text/html"
                        );
                        Transport.send(msg);

                        out.println("<h3 style='text-align: center; color:green;'>RESET USER PASSWORD MAIL SENT SUCCESSFULLY</h3>");
                    } catch(Exception e){
                        out.println("<h3 style='text-align: center; color:red;'>RESET USER PASSWORD MAIL SENT FAILED</h3>");
                    }
                } else{
